...
Database schemas describe data in terms of python types because pymongo
automatically converts to and from MongoDB data types. Each collection is
defined as a list of dictionaries, because that is native python's
...
Terminology:
    ...
    - "data schemas" are python dicts that are used to define the
      structure and/or validation rules concerning Python data structures. These
      structures may be constructed from any combination of dicts, lists, and
      primitive types in a nested fashion. Data schemas that define data
    ...
    - "valid keys" are the keys within a data schema that is the value of a
      _schema validation key. They define what string values are permitted as
...
Available validation keys:
    ...
    _schema  -  CAN ONLY BE USED WHEN "_data_type": dict.
                If omitted, any dict is valid regardless of its contents. If
                included, this validation key's value is a dict that specifies
                the form of the dict being defined. The keys of this
                dict-defining dict are the valid keys, together forming a
    ...
    _for_each - CAN ONLY BE USED WHEN "_data_type": list.
                ...
                valid regardless of its contents. If included, the value of
...
Key-value pairs for collection creation preferences:
    ...
    note: Indexed unique fields will be indexed together as a compound index and
          are together used to uniquely identify each peptide in peptides.db.
"""
