...
# Exclude all besides "experimental" activities
lamp.remove_rows_where_equals("found", "Predicted")
lamp.remove_rows_where_equals("found", "Patent")
lamp.remove_rows_where_equals("found", "patent")
# Separate activities into their own boolean fields
# Only using assume_false=True if there are many records with this property
lamp.create_bool_field_from_value("activities", "Antibacterial", assume_false=True)
lamp.create_bool_field_from_value("activities", "Antiviral", assume_false=True)
lamp.create_bool_field_from_value("activities", "Antifungal", assume_false=True)
lamp.create_bool_field_from_value("activities", "Antimicrobial", assume_false=True)
lamp.create_bool_field_from_value("activities", "Anticancer", assume_false=False)
lamp.create_bool_field_from_value("activities", "Antiparasitic", assume_false=False)
lamp.create_bool_field_from_value("activities", "Insecticidal", assume_false=False)
# field_names to lowercase
lamp.conform_field_names()
# Shown to determine if we should assume_false in our previoulsy created bool fields
...
lamp.remove_all_fields_except([k for k in definitions.collection_peptide["fields"]])
